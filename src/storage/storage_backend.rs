    ...
    pub async fn change_password(
        ...
        match self {
            StorageBackend::Browser(BrowserStorage::LocalStorage(local_storage)) => {
                ...
                let username = local_storage.get_username().expect("Failed to get username");

                // create a new LocalStorage instance with the old password
                let mut old_local_storage = LocalStorage::initiate_with_password(&username, old_password).await?;
...
mod tests {
    ...
    async fn test_change_password_local_storage() {
        ...
        let mut backend = StorageBackend::initiate_with_local_storage(username, Some(password))
            .await
            .expect("Failed to initiate local storage backend");
        ...
        match &mut backend {
            StorageBackend::Browser(BrowserStorage::LocalStorage(local_storage)) => {
                ...
                let item_meta = ItemMetaData::new_with_tags("item1", vec![("tag1".to_string(), "value1".to_string()), ("tag2".to_string(), "value2".to_string())].into_iter().collect());
                let content = b"Content of the item";
                local_storage.save_content(item_meta, content).await.expect("Failed to add item");
            }
        ...
        let new_backend = StorageBackend::initiate_with_local_storage(username, Some(new_password))
            .await
            .expect("Failed to initiate local storage backend");
        ...
        match new_backend {
            StorageBackend::Browser(BrowserStorage::LocalStorage(new_local_storage)) => {
                ...
                let items = new_local_storage.list_items().await.expect("Failed to list items");
                assert_eq!(items.len(), 1); // one item was added
                assert_eq!(items[0].id(), "item1");
                assert_eq!(items[0].tags(), Some(vec![("tag1".to_string(), "value1".to_string()), ("tag2".to_string(), "value2".to_string())].into_iter().collect()));

                // Check the content of the item
                let content = new_local_storage.load_content("item1").await.expect("Failed to load content");
